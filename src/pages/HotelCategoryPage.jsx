            ...
            >
              ...
              <div className="flex-1 overflow-y-auto overscroll-none scroll-smooth bg-white pb-16">
                ...
                <div className="relative h-[40vh] md:h-[50vh] w-full">
                  <img src={selectedHotel.image} alt={selectedHotel.name} draggable={false} className="w-full h-full object-cover select-none" />
                            ...
                            <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-4 -mx-6 px-6 md:mx-0 md:px-0 md:grid md:grid-cols-2 hide-scrollbar">
                              {selectedHotel.gallery.filter(img => img && img.trim() !== '').map((img, idx) => (
                                <div key={idx} className="w-[85vw] sm:w-[60vw] md:w-full flex-shrink-0 snap-center rounded-2xl md:rounded-3xl overflow-hidden aspect-[4/3] relative group shadow-sm">
                                  <img src={img} alt={`${selectedHotel.name} - Vue ${idx + 1}`} draggable={false} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 select-none" />
                        ...
                        >
                          <AnimatePresence>
                            {filteredGallery.map((img, idx) => (
                              ...
                              >
                                <img src={img.url} alt={img.type} draggable={false} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 select-none" />
