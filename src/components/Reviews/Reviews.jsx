      ...
      <div className="max-w-[1600px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {reviews.map((review) => (
            <div 
              key={review.id} 
              className="relative h-[450px] rounded-[2.5rem] overflow-hidden shadow-2xl group transition-all duration-700 hover:-translate-y-4 cursor-pointer"
            >
              {/* Background Image */}
              <img 
                src={review.image} 
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end text-white z-10">
                {/* Stars */}
                <div className="flex text-yellow-400 gap-1 mb-4 transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 delay-100">
                  {[...Array(5)].map((_, i) => (
